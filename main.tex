...
To test the BFP spectroscopy, we chose larger Au particles ($d=\SI{250}{\nano\meter}$), first and foremost for their much greater brightness in the dark field: 
...
Moreover, with a size close to the wavelengths under consideration, a perceptible change to the angular intensity distributions could be expected. 
Nevertheless, the scattering cross-section of one such AuNP could be expected to be smaller than that of a JP as used later-on, implying that if the measurement was valid for the AuNPs, it would also be for the JPs. 
%\begin{enumerate}[label=(\alph*)]
